...
public class ClaudeAiService {
    ...
    public JobExtractionResult extractJobFields(String rawText) {
        log.info("Sending job extraction request to Claude");
        String prompt = buildExtractionPrompt(rawText);
        String response = claudeClient.sendMessage(prompt);
        String json = extractJsonObject(response);

        try {
            log.debug("Successfully extracted JSON from Claude response");
            return objectMapper.readValue(json, JobExtractionResult.class);
        } catch (Exception e) {
            log.error("Failed to parse job extraction response as JSON", e);
    ...
    public GeneratedKitResult generateKit(Job job, UserProfile profile) {
        log.info("Sending kit generation request to Claude for job: {}", job.getTitle());
        String prompt = buildKitPrompt(job, profile);
        String response = claudeClient.sendMessage(prompt);
        String json = extractJsonObject(response);

        try {
            log.debug("Successfully extracted kit JSON from Claude response");
            return objectMapper.readValue(json, GeneratedKitResult.class);
        } catch (Exception e) {
            log.error("Failed to parse kit generation response as JSON", e);
